...
import { Calendar as CalendarIcon, Monitor, AlertCircle, Clock, User } from 'lucide-react';
...
export default function PaginaReservasLocal() {
  ...
  const slotsHorarios = ["08:00", "09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00"];
  ...
  const lidarComAgendamento = async (e: React.FormEvent) => {
    ...
    if (!nome || !email) {
      setMensagem({ tipo: 'erro', texto: 'Preencha o seu nome e e-mail acadêmico.' });
      return;
    }
  ...
  return (
    <main className="w-full bg-[#FAFAFA] min-h-screen pt-24 pb-20 font-sans">
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <h1 className="text-[#191F37] text-[42px] font-black mb-2">Reserva Interna de Equipamentos</h1>
        <p className="text-gray-600 mb-12">Gerenciamento direto do InPETU Maker (Sem dependência do Google Agenda).</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            {[1, 2, 3, 4].map((num) => (
              <button
                key={num}
                onClick={() => { setUnidadeAtiva(num); setMensagem({ tipo: '', texto: '' }); }}
                className={`w-full p-5 rounded-2xl border text-left flex justify-between items-center transition-all ${
                  unidadeAtiva === num ? 'border-[#0077cc] bg-blue-50/40 shadow-sm' : 'bg-white border-gray-100'
                }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-black ${
                    unidadeAtiva === num ? 'bg-[#0077cc] text-white' : 'bg-[#191F37] text-white'
                  }`}>
                    {num}
                  </div>
                  <div>
                ...
                <span className="text-[10px] uppercase font-black px-3 py-1 rounded-full bg-green-100 text-green-700">Livre</span>
              </button>
            ))}
            ...
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-md">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-100 pb-6 mb-6 gap-4">
                ...
                <input 
                  ...
                  className="border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold text-gray-700 outline-none focus:border-[#0077cc]"
                />
              </div>
              ...
              <div className="mb-8">
                ...
                {reservasDoDia.length === 0 ? (
                  <p className="text-sm text-green-600 font-medium bg-green-50 px-4 py-2 rounded-xl inline-block">🎉 Totalmente livre para este dia!</p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {reservasDoDia.map((r: any) => (
                      <span key={r.id} className="text-xs bg-red-50 text-red-700 font-bold border border-red-100 px-3 py-1.5 rounded-lg flex items-center gap-1.5">
                        <Clock size={12} /> {r.horario_inicio} até {r.horario_fim} ({r.usuario_nome})
                      </span>
                    ))}
                  </div>
                )}
